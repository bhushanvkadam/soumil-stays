...
const FAQS: FAQItem[] = [
  // Stay & Rooms
  {
    ...
    a: "Yes, Parvata Vista has a private swimming pool with no fixed usage-time restrictions — guests can use it at their leisure throughout the day.",
    category: "stay",
  },
  {
    q: "How many terraces does the villa have?",
    a: "Parvata Vista has two large terraces offering panoramic mountain views — ideal for morning tea, stargazing, or simply unwinding.",
    category: "stay",
  },
  {
    q: "Is food available at the property?",
    a: "This is a remote, secluded property surrounded by mountains. A local cook can prepare simple home-style meals at an additional charge, but food must be pre-arranged in advance — details of the cook are shared after booking is confirmed. The nearest restaurants are about 18 km away, with a Chinese food outlet roughly 7 km out. Guests are strongly advised to carry groceries, snacks, and essentials before arrival.",
    category: "stay",
  },
  {
    q: "Is there a kitchen?",
    a: "Yes, the villa has a basic kitchen with an induction cooktop and essential utensils. Guests are welcome to self-cook.",
    category: "stay",
  },
  {
    q: "Who is the caretaker?",
    a: "Caretaker and cook details are shared exclusively after the reservation is confirmed. Please enquire via WhatsApp to book.",
    category: "stay",
  },
  {
    q: "Is music allowed?",
    a: "Yes — Parvata Vista is a secluded property with no neighbours nearby, so music is permitted without strict time restrictions.",
    category: "stay",
  },
  {
    q: "How many bedrooms are there?",
    a: "Two air-conditioned bedrooms — 1 on the Ground Floor and 1 on the First Floor, each with an attached washroom.",
    category: "stay",
  },
  {
    q: "Can we bring pets?",
    a: "Pets are not permitted at Parvata Vista.",
    category: "stay",
  },
  // Bookings & Payment
  {
  ...
  // House Rules
  {
    q: "Is music allowed late at night?",
    a: "Yes — Parvata Vista is fully secluded with no nearby neighbours, so music is permitted without strict time restrictions. Guests are asked to be considerate of others in the group.",
    category: "rules",
  },
  {
  ...
  },
  {
    ...
    a: "Smoking is strictly prohibited inside the villa and all indoor areas. The terraces or a designated outdoor area may be used.",
    category: "rules",
  },
];
      ...
      >
        ...
        {!search && (
          <div className="flex border-b border-stone-100 px-6 flex-shrink-0">
            {CATEGORIES.map(({ id, label }) => (
              <button
                key={id}
                onClick={() => { setCategory(id); setOpenIndex(null); }}
        ...
        <div className="px-6 py-4 border-t border-stone-100 bg-[#F8F9FA] flex-shrink-0">
          <p className="text-xs text-gray-500 text-center">
            Have more questions?{" "}
            <a href={WA_LINK} target="_blank" rel="noopener noreferrer" className="text-forest font-medium hover:underline underline-offset-2">
